...
const fetchPage = async (url) => {
  ...
  if (!response.ok) {
    throw new Error(
      `Expected ${url} to return a 200 response, instead got ${response.status}`,
    );
  }
...
export const validateHreflang = async (url) => {
  ...
  for (const hreflang of hreflangs) {
    ...
    if (!validateIsoCode(hreflang))
      throw new Error(
        `"${hreflang}" is not the correct format or is not a valid language and locale combination`,
      );
  }
  ...
  const $altPages = await Promise.all(
    links.map(async (link) => ({ $altPage: await fetchPage(link), link })),
  );
